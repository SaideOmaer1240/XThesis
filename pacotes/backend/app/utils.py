...
from django.conf import settings
from ....backend.app.models import Topic, Thesis 
...
class Escriba:
    ...
    def escreva(self, tema, usuario, progress_callback): 
        print('Tema:', tema) 
        tema_do_trabalho, criado = Topic.objects.get_or_create(name=tema, author=usuario)
...
"""
        ...
        # Cadeia LLM para obter o esboço
        ...
        indices = outline_chain.invoke(tema)
        ...
        # Unir os elementos do JSON sumario em uma única string
        ...
        # Grupos de Cadeia LLM para obter os resultados desejados
        ...
        # Executar as cadeias Modelo de Linguagem Grande
        for titulo in indice:
            if "introdução" in titulo:
                ...
                resposta = chain1.invoke(tema)
                Thesis.objects.create(theme=tema_do_trabalho, title=titulo, text=resposta, author=usuario)
                progress_callback(f"{titulo} concluído com sucesso")
                time.sleep(30)
                ...
                texto = chain3.invoke(tema)
                resposta = self.regex_tirar_asteriscos(texto)
                Thesis.objects.create(theme=tema_do_trabalho, title=titulo, text=texto, author=usuario)
                progress_callback(f"{titulo} concluído com sucesso")
                time.sleep(30)
                ...
                resposta = self.regex_tirar_asteriscos(texto=chain5.invoke(tema))
                Thesis.objects.create(theme=tema_do_trabalho, title=titulo, text=resposta, author=usuario)
                progress_callback(f"{titulo} concluído com sucesso")
                time.sleep(30)
            elif titulo:
                ...
                texto = chain7.invoke(tema)
                resposta = self.regex_tirar_asteriscos(texto)
                Thesis.objects.create(theme=tema_do_trabalho, title=titulo, text=resposta, author=usuario)
                progress_callback(f"{titulo} concluído com sucesso")
                time.sleep(30)
            elif "Bibliografia" in titulo:
                chain8 = (
                    linha_de_saida_bibliografia
                    | model
                    | StrOutputParser()
                )
                texto = chain8.invoke(tema)
                resposta = self.regex_tirar_asteriscos(texto)
                Thesis.objects.create(theme=tema_do_trabalho, title=titulo, text=resposta, author=usuario)
                progress_callback(f"{titulo} concluído com sucesso")
                time.sleep(30)
    ...
    def regex_tirar_asteriscos(self, texto):
