...
def adicionar_ficha_ao_documento(doc, titulo, conteudo):
    ...
    # Adicionar parágrafos e imagens
    for parte in conteudo.split('\n'):
        url_imagem = extrair_url_imagem(parte)
        if url_imagem:
            adicionar_imagem(doc, url_imagem)
...
# Função para extrair URL de imagem de um texto
def extrair_url_imagem(texto):
    url_pattern = re.compile(r'(https?://\S+\.(?:jpg|jpeg|png|gif))')
    match = url_pattern.search(texto)
    return match.group(0) if match else None

# Função para adicionar uma imagem no documento
def adicionar_imagem(doc, url):
